...
const callbackRemoveStream = (connection, evt) => {
    ...
    var otherAudio = document.querySelector('#othervideo');
    otherAudio.src = '';
}
...
const callbackAddStream = (connection, evt) => {
    ...
    console.log(evt);
    var partnerAudio = document.querySelector('#othervideo');
    if (partnerAudio.srcObject !== evt.streams[0]) {
        partnerAudio.srcObject = evt.streams[0];
        console.log("OnPage: Attached remote stream");
        //partnerAudio.addTrack(evt.track);
    }
}
...
const askUsername = () => {
    ...
    wsconn.invoke("Join", $("#upperUsername").text()).catch((err) => {
        consoleLogger(err);
        alertify.alert('<h4>Failed SignalR Connection</h4> We were not able to connect you to the signaling server.<br/><br/>Error: ' + JSON.stringify(err));
        //viewModel.Loading(false);
    });
